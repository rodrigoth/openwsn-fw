...
void ranking_timer_cb(opentimer_id_t id);
void ranking_task_cb(void);
void timer_ranking_fired(void);
...
void ranking_startPeriodRanking() {
    if (ranking_vars.timeStarted == FALSE) {
		ranking_vars.timerId = opentimers_start(RANKING_FIRST_OBSERVATION_PERIOD,TIMER_PERIODIC,TIME_MS,ranking_timer_cb);
...
void ranking_timer_cb(opentimer_id_t id){
  	//openserial_printError(COMPONENT_RANKING,ERR_SORT, (errorparameter_t)ranking_vars.timerId,ranking_vars.timerId);	
  	scheduler_push_task(timer_ranking_fired,TASKPRIO_SF0);	
}

void timer_ranking_fired() {
	ranking_task_cb();
	opentimers_setPeriod(ranking_vars.timerId,TIME_MS,RANKING_OBSERVATION_WINDOW_PERIOD_MS);
}
