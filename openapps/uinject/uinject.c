...
void uinject_task_cb() {
   ...
   open_addr_t *my_address_16B = idmanager_getMyID(ADDR_16B);
	if (my_address_16B->addr_16b[0] == m3_103[6]
			&& my_address_16B->addr_16b[1] == m3_103[7]) {
		...
		memset(&(ipAddr_node[0]), 0x00, sizeof(ipAddr_node));
		memcpy(&(ipAddr_node[0]), &all_routers_multicast,
		...
		if (pkt == NULL) {
			openserial_printError(COMPONENT_UINJECT, ERR_NO_FREE_PACKET_BUFFER,
					(errorparameter_t) 1, (errorparameter_t) 1);
			return;
		}
